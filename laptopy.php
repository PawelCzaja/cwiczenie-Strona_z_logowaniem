<HTML>
    ...
    <body>
        ...
        <p class="title">LAPTOPY</p>
        <div class="placeholder2"></div>
        <div id="k_midder">
            <div id="midder0">
                <form method="post" action="laptopy.php">
                    <h2>FILTRY</h2><br>
                    <p> Zakres ceny </p>
...
$conn = new mysqli($servername, $username, $password, "strona");
    ...
    if(isset($_POST['min_cena']) || isset($_POST['max_cena']) || isset($_POST['Apple']) || isset($_POST['Dell']) || isset($_POST['Microsoft']))
    {
        ...
        if(isset($_POST['Dell']) || isset($_POST['Microsoft']) || isset($_POST['Apple']))
        {
            ...
            $sql_notatki = 'SELECT `nazwa`, `marka`, `cena`, `opis`, `image` FROM `laptopy` WHERE (laptopy.cena BETWEEN '.$min_cena.' AND '.$max_cena.' ) AND (`marka` LIKE "'.$marka2.'" OR `marka` LIKE "'.$marka1.'" OR `marka` LIKE "'.$marka3.'") ORDER BY `cena` ASC;;';
        }
        else
        {
            $sql_notatki = 'SELECT `nazwa`, `marka`, `cena`, `opis` , `image` FROM `laptopy` WHERE laptopy.cena BETWEEN '.$min_cena.' AND '.$max_cena.' ORDER BY `cena` ASC;;';
        }
...
{
$sql_notatki = 'SELECT  `nazwa`, `marka`, `cena`, `opis` FROM `laptopy` ORDER BY `cena` ASC;;';
$notatki = mysqli_query($conn, $sql_notatki);
$ile = mysqli_num_rows($notatki);
for($i=0;$i<=$ile;$i++)
{
